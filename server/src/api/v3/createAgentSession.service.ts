  ...
  async create(
    ...
    const user =
      await this.prisma.user.findUnique({
        where: {
          agentJWT:
            createAgentSessionDto.agentAuthenticationKey,
        },
      });

    if (!user) {
      throw new UnauthorizedException(
        'Invalid agent authentication key',
      );
    }
    ...
    if (existingSessions.length > 0) {
      for (const existingSession of existingSessions) {
        console.log(
          `Existing sessions deleted at ${new Date()}\n`,
          existingSession,
        );
    ...
    return {
      agentAuthorizationToken: authorizationToken,
    };
  }
}
