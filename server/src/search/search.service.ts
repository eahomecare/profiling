  ...
  implements OnModuleInit
{
  ...
  async createAutocompleteIndex(index: string) {
    try {
      ...
      console.error(
        'Error creating autocomplete index:',
        error,
      );
      throw error;
    }
  ...
  async autocomplete(
    ...
    try {
      ...
      return response.body.suggest.label_suggestion[0].options.map(
        (option) => ({
          id: option._id,
          ...option._source,
        }),
  ...
  async autocompleteBothFields(
    ...
    try {
      ...
      const categorySuggestions =
        categoryResponse.body.suggest.label_suggestion[0].options.map(
          (option) => ({
            id: option._id,
            ...option._source,
          }),
      ...
      const valueSuggestions =
        valueResponse.body.suggest.label_suggestion[0].options.map(
          (option) => ({
            id: option._id,
            ...option._source,
          }),
      ...
      console.error(
        'Error during autocomplete for both fields:',
        error,
      );
      throw error;
    }
